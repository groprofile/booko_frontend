  ...
  return (
    <section>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <span className="text-sm text-[#64748B]">({reviewCount.toLocaleString()} Reviews)</span>
      </div>
      ...
      <div className="mt-4 flex flex-col gap-3">
        {visibleReviews.length === 0 ? (
          <p className="text-sm text-[#64748B]">No reviews yet for this traveler type.</p>
        ) : (
          visibleReviews.map((review, index) => (
            <div key={`${review.name}-${index}`} className="rounded-[16px] border border-[#E2E8F0] bg-white p-4">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="text-[#94A3B8]">{review.travelerType}</span>
              </div>
