...
import type { BillingTier, MembershipType } from "../../data/monthlyPassDetails";
...
export default function MobileBottomBar({ membershipTypes, billingTiers, listing }: MobileBottomBarProps) {
  const [open, setOpen] = useState(false);
  const startingType = membershipTypes[0];
    ...
    <>
      <div className="fixed inset-x-0 bottom-0 z-40 flex items-center justify-between gap-3 border-t border-[#E2E8F0] bg-white px-4 py-3 shadow-[0_-8px_24px_rgba(15,23,42,0.08)] sm:hidden">
        <div>
          ...
          <p className="text-lg font-extrabold text-[#0F172A]">₹{startingType.price.toLocaleString()}/mo</p>
        </div>
        <button
          type="button"
      ...
      {open && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center sm:hidden">
          ...
          <div className="relative max-h-[90vh] w-full overflow-y-auto rounded-t-[24px] bg-[#F8FAFC] p-4">
            ...
            <BookingCard membershipTypes={membershipTypes} billingTiers={billingTiers} listing={listing} />
          </div>
        </div>
      )}
    </>
